...
int UpdateTensorVec() {
    ...
    for (auto inDim : inputDimension) {
        ...
        IF_BOOL_EXEC(ret != SUCCESS,
                     LOGE("[HIAI_DEMO_SYNC] model %s input AiTensor Init failed(input).",
                          modelName);
                             return FAILED);
        inputTensor.push_back(input);
    }
    IF_BOOL_EXEC(inputTensor.size() == 0,
                 LOGE("[HIAI_DEMO_SYNC] inputTensor.size() == 0"); return FAILED);

    for (auto outDim : outputDimension) {
        shared_ptr<AiTensor> output = make_shared<AiTensor>();
        int ret = output->Init(&outDim);
        IF_BOOL_EXEC(ret != SUCCESS,
                     LOGE("[HIAI_DEMO_SYNC] model %s output AiTensor Init failed(output).",
                          modelName);
                             return FAILED);
        outputTensor.push_back(output);
    }
    IF_BOOL_EXEC(outputTensor.size() == 0,
                 LOGE("[HIAI_DEMO_SYNC] outputTensor.size() == 0"); return FAILED);
...
shared_ptr<AiModelMngerClient> LoadModelSync(const char *modelPath) {
    ...
    IF_BOOL_EXEC(clientSync == nullptr,
                 LOGE("[HIAI_DEMO_SYNC] Model Manager Client make_shared error.");
                         return nullptr);
    int ret = clientSync->Init(nullptr);
    IF_BOOL_EXEC(ret != SUCCESS,
                 LOGE("[HIAI_DEMO_SYNC] Model Manager Init Failed."); return nullptr);
    ...
    IF_BOOL_EXEC(buffer == nullptr,
                 LOGE("[HIAI_DEMO_SYNC] cannot find the model file."); return nullptr);
    ...
    LOGI("[HIAI_DEMO_SYNC] loadModel %s IO Tensor.", desc->GetName().c_str());
    ...
    ret = clientSync->GetModelIOTensorDim(string(modelName) + string(".om"), inputDimension,
                                          outputDimension);
    IF_BOOL_EXEC(ret != SUCCESS,
                 LOGE("[HIAI_DEMO_SYNC] Get Model IO Tensor Dimension failed,ret is %d.", ret);
                         return nullptr);
    IF_BOOL_EXEC(inputDimension.size() == 0,
                 LOGE("[HIAI_DEMO_SYNC] inputDimension.size() == 0"); return nullptr);
...
extern "C" JNIEXPORT jobject JNICALL Java_com_huawei_hiaidemo_utils_ModelManager_loadModelSync(
    ...
    jmethodID getOfflineModelName = env->GetMethodID(modelInfoClass, "getOfflineModelName",
                                                     "()Ljava/lang/String;");
    jmethodID getModelPath = env->GetMethodID(modelInfoClass, "getModelPath",
                                              "()Ljava/lang/String;");
    IF_BOOL_EXEC(getOfflineModelName == nullptr,
    ...
    IF_BOOL_EXEC(modelName == nullptr,
                 LOGE("[HIAI_DEMO_SYNC] modelName is invalid."); return nullptr);
    const char *modelPath = env->GetStringUTFChars(strModelPath, 0);
    IF_BOOL_EXEC(modelPath == nullptr,
                 LOGE("[HIAI_DEMO_SYNC] modelPath is invalid."); return nullptr);
    // load
    IF_BOOL_EXEC(!g_clientSync, g_clientSync = LoadModelSync(modelPath); IF_BOOL_EXEC(
            g_clientSync == nullptr,
            LOGE("[HIAI_DEMO_SYNC] g_clientSync loadModel is nullptr."); return nullptr));
    // load model
...
int runJniProcess(JNIEnv *env, jobject bufList, jmethodID listGet, int len) {
    LOGI("[HIAI_DEMO_SYNC] INPUT NCHW : %d %d %d %d.", inputDimension[0].GetNumber(),
         inputDimension[0].GetChannel(),
         inputDimension[0].GetHeight(), inputDimension[0].GetWidth());
    LOGI("[HIAI_DEMO_SYNC] OUTPUT NCHW : %d %d %d %d.", outputDimension[0].GetNumber(),
         outputDimension[0].GetChannel(),
...
extern "C" JNIEXPORT jobject JNICALL Java_com_huawei_hiaidemo_utils_ModelManager_runModelSync(
    ...
    IF_BOOL_EXEC(ModelInfo == nullptr,
                 LOGE("[HIAI_DEMO_SYNC] can not find ModelInfo class."); return nullptr);
    IF_BOOL_EXEC(bufList == nullptr, LOGE("[HIAI_DEMO_SYNC] buf_ is null."); return nullptr);
    jmethodID getOfflineModelName = env->GetMethodID(ModelInfo, "getOfflineModelName",
                                                     "()Ljava/lang/String;");
    ...
    IF_BOOL_EXEC(classList == nullptr,
                 LOGE("[HIAI_DEMO_SYNC] can not find List class."); return nullptr);
    jmethodID listGet = env->GetMethodID(classList, "get", "(I)Ljava/lang/Object;");
    jmethodID listSize = env->GetMethodID(classList, "size", "()I");
    IF_BOOL_EXEC(listGet == nullptr,
                 LOGE("[HIAI_DEMO_SYNC] can not find get method."); return nullptr);
    IF_BOOL_EXEC(listSize == nullptr,
                 LOGE("[HIAI_DEMO_SYNC] can not find size method."); return nullptr);

    int len = static_cast<int>(env->CallIntMethod(bufList, listSize));
    // load
    IF_BOOL_EXEC(!g_clientSync, g_clientSync = LoadModelSync(modelPath); IF_BOOL_EXEC(
            g_clientSync == nullptr,
            LOGE("[HIAI_DEMO_SYNC] Model Manager Client is nullptr."); return nullptr));
    // run
