...
bool readAsset(
    std::string valuePath, uint32_t inChannels, uint32_t outChannels, uint32_t h, uint32_t w,
    vector<float>& output) {
    ...
    if (assetLength != length * sizeof(float)) {
        ...
        ALOGE("weight file length is %zu ,but shape length is %zu", assetLength,
              length * sizeof(float));
        return false;
    }
...
bool readWeight(string layerName, int inChannels, int outChannels, int h, int w,
...
bool
readBias(string layerName, int inChannels, int outChannels, int h, int w, vector<float>& bias) {
    string tmpName = layerName;
    replace(tmpName.begin(), tmpName.end(), '/', '@');
...
bool getConvRealWeight(
    string layerName, ge::AttrValue::LIST_TENSOR& weight, int inChannels, int outChannels, int h,
    int w) {
    ...
    if (readWeight(layerName, inChannels, outChannels, h, w, dataValuec)) {
        filter->SetData(
            reinterpret_cast<uint8_t*>(dataValuec.data()),
            w * h * inChannels * outChannels * sizeof(float));
...
void getConvdata(hiai::op::Data& data, string opName, int inChannels, int outChannels, int h, int w,
                 ge::Format format,
...
bool buildSqueezenetGraph(Graph& graph) {
    ...
    hiai::op::Const conv10_const_1 = hiai::op::Const("conv10_const_1").set_attr_value(
        weightList0[0]);
    ...
    hiai::op::Const conv10_const_0 = hiai::op::Const("conv10_const_0").set_attr_value(
        weightList1[0]);
    ...
    hiai::op::Const fire9_expand3x3_const_1 = hiai::op::Const(
        "fire9/expand3x3_const_1").set_attr_value(weightList2[0]);

    ge::AttrValue::LIST_TENSOR weightList3;
    if (!getConvRealWeight("fire9/expand3x3", weightList3, 256, 64, 3, 3)) {
        return false;
    }
    hiai::op::Const fire9_expand3x3_const_0 = hiai::op::Const(
        "fire9/expand3x3_const_0").set_attr_value(weightList3[0]);

    ge::AttrValue::LIST_TENSOR weightList4;
    if (!getConvRealBias("fire9/expand1x1", weightList4, 1, 256, 1, 1)) {
        return false;
    }
    hiai::op::Const fire9_expand1x1_const_1 = hiai::op::Const(
        "fire9/expand1x1_const_1").set_attr_value(weightList4[0]);

    ge::AttrValue::LIST_TENSOR weightList5;
    if (!getConvRealWeight("fire9/expand1x1", weightList5, 256, 64, 1, 1)) {
        return false;
    }
    hiai::op::Const fire9_expand1x1_const_0 = hiai::op::Const(
        "fire9/expand1x1_const_0").set_attr_value(weightList5[0]);
    ...
    hiai::op::Const fire8_expand3x3_const_1 = hiai::op::Const(
        "fire8/expand3x3_const_1").set_attr_value(weightList8[0]);

    ge::AttrValue::LIST_TENSOR weightList9;
    if (!getConvRealWeight("fire8/expand3x3", weightList9, 256, 64, 3, 3)) {
        return false;
    }
    hiai::op::Const fire8_expand3x3_const_0 = hiai::op::Const(
        "fire8/expand3x3_const_0").set_attr_value(weightList9[0]);
    ...
    hiai::op::Const conv1_const_1 = hiai::op::Const("conv1_const_1").set_attr_value(
        weightList50[0]);
    ...
    hiai::op::Const conv1_const_0 = hiai::op::Const("conv1_const_0").set_attr_value(
        weightList51[0]);
    ...
    auto relu_conv1 = hiai::op::Activation("relu_conv1").set_input_x(conv1).set_attr_coef(
        0.000000).set_attr_mode(1);
    ...
    auto conv10 = hiai::op::Convolution("conv10")
        ...
        .set_attr_pads(AttrValue::LIST_INT({0, 0, 0, 0}))
        .set_attr_strides(AttrValue::LIST_INT({1, 1}));
    auto relu_conv10 = hiai::op::Activation("relu_conv10").set_input_x(conv10).set_attr_mode(
        1).set_attr_coef(0.000000);
...
Java_com_huawei_hiaidemo_utils_ModelManager_createOmModel(
